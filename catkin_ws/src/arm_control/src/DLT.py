...
class DLT:
    ...
    def DLT_Load_From_File(self, filename):
        ...
        self.calibrationPointsXYZ = np.array(
            [[float(x) for x in f.readline().split()] for i in range(self.numPoints)]
        )
    ...
    def DLT_Save_To_File(self):
        ...
        filename = (
            folder
            + "/"
            + self.left.split("/")[-1].split(".")[0]
            + "_"
            + self.right.split("/")[-1].split(".")[0]
            + ".txt"
        )
    ...
    def getCalibrationPoints(self):
        ...
        for i in range(self.numPoints):
            ...
            cv2.putText(
                left,
                f"Select calibration point {i} on the left image then press a key to continue to the next point",
                (10, 10),
        ...
        for i in range(self.numPoints):
            ...
            cv2.putText(
                right,
                f"Select calibration point {i} on the right image then press a key to continue to the next point",
                (10, 10),
                cv2.FONT_HERSHEY_COMPLEX_SMALL,
                2,
                (0, 0, 255),
        ...
        for i in range(self.numPoints):
            prompt = input(
                f"Enter in units of cm the X,Y,Z coordinates of calibration point {i} as X Y Z: "
            )
            x, y, z = prompt.split()
    ...
    def solveLR(self):
        ...
        self.XL = np.hstack(
            (
                self.calibrationPointsXYZ,
                self.tu,
                (-self.uL * self.calibrationPointsXYZ.conj().T[0, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.uL * self.calibrationPointsXYZ.conj().T[1, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.uL * self.calibrationPointsXYZ.conj().T[2, :]).reshape(
                    self.numPoints, 1
                ),
            )
        )
        ...
        self.YL = np.hstack(
            (
                ...
                self.calibrationPointsXYZ,
                self.tv2,
                (-self.vL * self.calibrationPointsXYZ.conj().T[0, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.vL * self.calibrationPointsXYZ.conj().T[1, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.vL * self.calibrationPointsXYZ.conj().T[2, :]).reshape(
                    self.numPoints, 1
                ),
            )
        )
        ...
        self.XR = np.hstack(
            (
                self.calibrationPointsXYZ,
                self.tu,
                (-self.uR * self.calibrationPointsXYZ.conj().T[0, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.uR * self.calibrationPointsXYZ.conj().T[1, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.uR * self.calibrationPointsXYZ.conj().T[2, :]).reshape(
                    self.numPoints, 1
                ),
            )
        )
        self.YR = np.hstack(
            (
                ...
                self.calibrationPointsXYZ,
                self.tv2,
                (-self.vR * self.calibrationPointsXYZ.conj().T[0, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.vR * self.calibrationPointsXYZ.conj().T[1, :]).reshape(
                    self.numPoints, 1
                ),
                (-self.vR * self.calibrationPointsXYZ.conj().T[2, :]).reshape(
                    self.numPoints, 1
                ),
            )
        )
    ...
    def showCalibrationPoints(self):
        ...
        for i in range(self.numPoints):
            ...
            for j in range(self.uL.shape[0]):
                ...
                xyz = self.calcXYZBasedOnPixels(
                    self.uL[j], self.vL[j], self.uR[j], self.vR[j]
                ).reshape(3, 1)
    ...
    def getXYZ(self):
        ...
        left = cv2.imread(self.left)
        # left = cv2.resize(left, (600, 600))
        cv2.putText(
            left,
            f"Select the point to measure",
            (10, 10),
            cv2.FONT_HERSHEY_COMPLEX_SMALL,
            0.5,
            (0, 0, 255),
        ...
        right = cv2.imread(self.right)
        # right = cv2.resize(right, (600, 600))
        cv2.putText(
            right,
            f"Select the point to measure",
            (10, 10),
...
if __name__ == "__main__":
    ...
    dltobj.DLT_Load_From_File(
        "/Users/ikas/Documents/ME 537/Robotics_Final_Project/DLT_Data/left_with_robot_right_with_robot.txt"
    )
